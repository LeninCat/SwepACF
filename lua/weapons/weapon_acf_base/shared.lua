...
function SWEP:VisRecoil()
	...
	if self:Clip1() == self.LastAmmoCountAppliedRecoil then return end
        ...
        local punchScale = self.RecoilScale * 5
		...
		local lolangle = Angle(rnda,rndb,0)
		
		self.Owner:ViewPunch(lolangle) 
		
	if CLIENT then
		local lorangle = Angle(rnda,rndb,0)
        local aimAng = self.Owner:EyeAngles()
		aimAng.pitch = aimAng.pitch + lorangle.pitch/math.Clamp( (self.Owner:Ping()/30), 2, 20 )
		aimAng.yaw = aimAng.yaw + lorangle.yaw/math.Clamp( (self.Owner:Ping()/30), 2, 20 )
		self.Owner:SetEyeAngles(aimAng)
	end
end
